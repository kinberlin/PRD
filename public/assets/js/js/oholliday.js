$(document).ready(function () {
    ...
    $(document).ready(function () {
        ...
        var selectedOption = $("#lsuser").find("option:selected");
        var extraInfo = selectedOption.data("extra-info2");
        $("#pneduration").attr("placeholder", "La permission ne pourra pas aller au-dela de : " +
            extraInfo + " Jours");
        $("#pneduration").attr("max", extraInfo);
        // for matricule
        var benef = $("#lsuser").find("option:selected");
        var mats = benef.data("extra-info");
        $("#empmatricule").val(mats);

        $("#lsuser").change(function () {
            var benef = $(this).find("option:selected");
            var mats = benef.data("extra-info");
            $("#empmatricule").val(mats);
            var dur = benef.data("extra-info2");
            $("#pneduration").attr("placeholder", "La permission ne pourra pas aller au-dela de : " +
                dur + " Jours");
            $("#pneduration").attr("max", dur);

        });

        $("#pneduration").on('input', function () {
            ...
            if ($.isNumeric(enteredValue)) {
                ...
                if (numericValue < 1 || numericValue > maxValue) {
                    $(this).val('');
                    alert("Entrer un chiffre compris entre 12 et " + maxValue
                    );
                }
            } else {
                $(this).val('');
