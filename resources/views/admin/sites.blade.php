                        ...
                        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEnd"
                            ...
                            <div class="offcanvas-body mx-0 flex-grow-0">
                                <form class="add-new-user pt-0 fv-plugins-bootstrap5 fv-plugins-framework"
                                    ...
                                    <div class="mb-3 fv-plugins-icon-container">
                                        <label class="form-label" for="place">Emplacement du Site</label>
                    ...
                    <table id="datatables-orders"
                        ...
                        <tbody>
                            @foreach ($data as $d)
                                <tr>
                                    <td>{{ $d->id }}</td>
                                    <td>{{ $d->enterprise }} | {{$ents->where('id', $d->enterprise)->first()->name}}</td>
                                    <td>{{ $d->name }}</td>
                                    <td>{{ $d->location }}</td>
                                    <td> <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal"
                                        ...
                                        <div class="modal animate__animated animate__bounceInUp"
                                            id="majsite{{ $d->id }}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <form class="modal-content" action="/admin/department/{{ $d->id }}"
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="row">
                                                            <div class="col mb-3">
                                                                <label for="location{{$d->id}}" class="form-label">Emplacement</label>
                                                                <input type="text" id="location{{$d->id}}" name="location"
                                                                    value="{{ $d->location }}" class="form-control"
                                                                    placeholder="Entrer le nom">
                                                            </div>
                                                        </div>
                                                        <div class="row">
                                                            <div class="col mb-3">
                                                                <label for="entsname{{$d->id}}" class="form-label">Choisissez
                                                                    l'Entreprise</label>
                                                                <select name="enterprise" id="entsname{{$d->id}}" class="form-select" required
                                                                    readonly disabled>
                                                                    @foreach ($ents as $e)
